...
public final class StaffWorld extends CustomWorld
{
    ...
    private static final String WORLD_NAME = "staffworld";
    //
    ...
    protected World generateWorld()
    {
        ...
        welcomeSign.setLine(0, ChatColor.DARK_GRAY + "StaffWorld");
    ...
    {
        if (guest == supervisor || plugin.al.isStaffMember(guest))
        {
            return false;
        }

        if (plugin.al.isStaffMember(supervisor))
        {
    ...
    public boolean canAccessWorld(final Player player)
    {
        ...
        if (cached == null)
        {
            ...
            if (!canAccess)
            {
                ...
                canAccess = supervisor != null && supervisor.isOnline() && plugin.al.isStaffMember(supervisor);
                if (!canAccess)
                {
